...
def play_game(player, entities, game_map, message_log, game_state, root_console, con, panel, constants):
    ...
    while not tdl.event.is_window_closed():
        ...
        render_all(con, panel, entities, player, game_map, fov_recompute, root_console, message_log,
                   ...
                   constants["panel_height"], constants["panel_y"], mouse_coordinates, constants["colors"], game_state)
        tdl.flush()
        ...
        if move and game_state == GameStates.PLAYERS_TURN:
            ...
            for entity in entities:
                ...
                message_log.add_message(Message("Nothing to pick up.", constants["colors"].get("yellow")))
        ...
        for player_turn_result in player_turn_results:
            ...
            if equip:
                ...
                for equip_result in equip_results:
                    ...
                    if equipped:
                        message_log.add_message(Message("Equipped {0}.".format(equipped.name)))

                    if dequipped:
                        message_log.add_message(Message("Removed {0}.".format(dequipped.name)))
            ...
            if xp:
                ...
                message_log.add_message(Message("{0} experience gained.".format(xp)))

                if leveled_up:
                    #todo take another shot at the level up message
                    message_log.add_message(Message(
                        "Your effort pays off, and you reach level {0}!".format(player.level.current_level),
