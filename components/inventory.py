...
class Inventory:
    ...
    def add_item(self, item, colors):
        ...
        if len(self.items) >= self.capacity:
            results.append({
                "item_added": None,
                "message": Message("You're already carrying too much to take this!", colors.get("yellow"))
            })
        else:
            results.append({
                "item_added": item,
                "message": Message("You take the {0}.".format(item.name), colors.get("dark_cyan"))
            })
    ...
    def use(self, item_entity, colors, **kwargs):
        ...
        if item_component.use_function is None:
            results.append({"message": Message("You can't use that like this.", colors.get("yellow"))})
        else:
